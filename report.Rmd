...
Pretpostavke modela: - Linearnost: odnos između nezavisnih i zavisne varijable je linearni - Pogreške su nezavisne homogene i normalno distribuirane
...
```{r categorical_correlation_matrix}
# Create all dummy variables first
variables <- music_data %>%
  select(Age, `Primary streaming service`, `Exploratory`, `While working`, `Composer`)

dummyvars <- dummy_cols(variables, remove_selected_columns = TRUE) %>%
  select(Age,
         contains("_Youtube Music"),
         contains("_Spotify"),
         contains("_Apple"),
         contains("_None"),
         contains("Exploratory_"),
         contains("While working_"),
         contains("Composer_"))

# Calculate and visualize correlation matrix
correlation_matrix <- cor(dummyvars, method = "spearman", use = "pairwise.complete.obs")
corrplot(correlation_matrix, method = "color", type = "lower", tl.col = "black",
         tl.cex = 0.7, col = colorRampPalette(c("blue", "white", "red"))(200))
```

Prema korelacijskim tablicama odlučili smo uzeti varijable `Primary streaming service`, `Hours per day`, `Exploratory`, `Depression`, `While working`, `Composer`.

```{r linear_regression_depression}
model <- lm(Age ~ Depression, data = music_data)
...
# Funkcija za konverziju frequency varijabli u numeričke
...
prepare_data <- function(df) {
  ...
  df_prep <- df %>%
    # Izbacujemo Music effects i log_Age
    select(-c(`Music effects`, log_Age)) %>%
    ...
    mutate(
      ...
      Instrumentalist = as.factor(Instrumentalist),
      Composer = as.factor(Composer),
      `Fav genre` = as.factor(`Fav genre`),
      Exploratory = as.factor(Exploratory),
      `Foreign languages` = as.factor(`Foreign languages`)
    )
  
  # Konverzija frequency varijabli
  frequency_cols <- grep("Frequency", names(df_prep), value = TRUE)
  for(col in frequency_cols) {
    df_prep[[col]] <- convert_frequency(df_prep[[col]])
  }
  
  # Kreiranje dummy varijabli za kategoričke varijable
  categorical_cols <- c("Primary streaming service", "While working", 
                       "Instrumentalist", "Composer", "Fav genre", 
                       "Exploratory", "Foreign languages")
  
  dummy_data <- dummyVars("~ .", data = df_prep[categorical_cols])
  dummy_matrix <- predict(dummy_data, df_prep[categorical_cols])
  
  # Priprema numeričkih varijabli
  numeric_data <- df_prep %>%
    select(Age, `Hours per day`, BPM, all_of(frequency_cols),
           Anxiety, Depression, Insomnia, OCD)
  
  # Standardizacija numeričkih varijabli (osim Age)
...
# Funkcija za analizu
analyze_age_prediction <- function(df) {
  # Priprema podataka
  data_final <- prepare_data(df)
  
  # Treniranje modela na svim podacima
  model <- lm(Age ~ ., data = data_final)
  
  # Predikcije na istim podacima (za vizualizaciju)
  predictions <- predict(model, newdata = data_final)
  
  # Analiza važnosti varijabli
  ...
  # Vizualizacija najvažnijih faktora
  top_10_plot <- ggplot(importance %>% head(10), 
                        aes(x = reorder(Feature, Importance), y = Importance)) +
    geom_bar(stat = "identity") +
    coord_flip() +
    theme_minimal() +
    labs(title = "Top 10 najvažnijih faktora za predikciju dobi",
         x = "Faktori",
         y = "Važnost")
  
  # Vizualizacija stvarnih vs. predviđenih vrijednosti
  prediction_plot <- ggplot(data.frame(Actual = data_final$Age, 
                                     Predicted = predictions), 
                           aes(x = Actual, y = Predicted)) +
    geom_point(alpha = 0.5) +
    geom_abline(intercept = 0, slope = 1, color = "red") +
    theme_minimal() +
    labs(title = "Stvarne vs. Predviđene vrijednosti",
         x = "Stvarna dob",
         y = "Predviđena dob")
...
# Pokretanje analize
results <- analyze_age_prediction(music_data)

# Ispis rezultata
print(paste("R2:", round(results$r2, 3)))

# Prikaz vizualizacija
print(results$top_10_plot)
print(results$prediction_plot)

# Pregled najvažnijih faktora
print("Najvažniji faktori:")
print(head(results$importance, 10))

# Detaljan summary modela
